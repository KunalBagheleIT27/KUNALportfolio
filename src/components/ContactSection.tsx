    ...
    <section id="contact" ref={ref} className="relative py-20 md:py-32 px-4 sm:px-6 md:px-10 overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12" data-reveal>
          {/* Email card */}
          <div className="group relative border border-border bg-card/50 backdrop-blur-sm p-6 sm:p-8 hover:border-lime/50 transition-all duration-500 md:col-span-2"
            style={{ perspective: "600px" }}>
            <div className={`absolute inset-0 bg-gradient-to-br from-lime/5 to-transparent transition-opacity duration-500 ${isMobile ? "opacity-40" : "opacity-0 group-hover:opacity-100"}`} />
          ...
          {/* Location card */}
          <div className="group relative border border-border bg-card/50 backdrop-blur-sm p-6 sm:p-8 hover:border-ice/50 transition-all duration-500">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8" data-reveal>
          {socials.map((s) => (
            <a key={s.name} href={s.href} data-hover aria-label={s.name}
              ...
              className={`group relative h-20 border bg-card/30 flex items-center justify-center gap-3 transition-all duration-300 overflow-hidden ${hoveredSocial === s.name || isMobile ? "bg-lime border-lime" : "border-border hover:bg-lime hover:border-lime"}`}
              style={{ perspective: "400px" }}
            >
              <div className={`absolute inset-0 bg-gradient-to-t from-lime/10 to-transparent transition-opacity ${hoveredSocial === s.name || isMobile ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`} />
        ...
        <div data-reveal style={{ perspective: "600px" }}>
          <a href="mailto:hello@kunalbaghele.dev?subject=Resume%20Request&body=Hi%20Kunal%2C%20please%20share%20your%20latest%20resume." data-hover
